function downloadFile(metadata, folder) {
    ...
    g_workingDir.createFileAsync(metadata.name, Windows.Storage.CreationCollisionOption.replaceExisting).done(function (myfile) {
        myfile.openAsync(Windows.Storage.FileAccessMode.readWrite).done(function (output) {
            downloadChunks(metadata, folder, g_complete, new Windows.Storage.Streams.DataWriter(output.getOutputStreamAt(0)));
        });
    });
}

function downloadChunks(metadata, folder, chunkIdx, writer) {
    var i;
    g_chunks = [];
    for (i = 0; i < g_providers.length; i++) {
        dropboxDownload(metadata, folder, chunkIdx + i, g_providers[1].token, writer);
    }
}

function downloadComplete(metadata, folder, writer) {
    var debug = $('#debug');
    var i, nbRead = 0;
    g_complete++;
    ...
    if (g_complete % g_providers.length == 0) {
        ...
        if (g_complete < metadata['chunks'].length) {
            downloadChunks(metadata, folder, g_complete, writer);
        } else {
...
function downloadConfiguration() {
    ...
    downloadChunks(metadata, undefined, g_complete, writer);
}
